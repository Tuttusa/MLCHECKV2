                            ...
                            if df.columns.values[j]+fe_add in file_content[i]:
                                ...
                                if 'Int' in file_content[i] and not cvc_flag:
                                    ...
                                    if "(/" in file_content[i]:
                                        if('-' in file_content[i]):
                                            minus_flag = True
                                        multi_digits = re.findall('[+-]?([0-9]*[.])?[0-9]+', file_content[i])
                                        if len(multi_digits) == 1:
                                            i = i+1
                                            multi_digits.append(float(re.search(r'[+-]?([0-9]*[.])?[0-9]+', file_content[i]).group(0)))
                                       ...
                                       # digit = format(digit, '.5f')
                                        ...
                                        digit = float(re.search(r'[+-]?([0-9]*[.])?[0-9]+', file_content[i]).group(0))
                                        #digit = format(digit, '.5f')
                                        if('-' in file_content[i]):
                                            digit = 0-digit
